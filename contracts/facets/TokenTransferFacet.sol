...
contract TokenTransferFacet is ITokenTransferFacet {
    ...
    event ProposalCreated(
        ...
        LibVoteStorage.Proposal proposal
    );
    ...
    function createProposal(
        ...
        require(
            vs.proposal[proposalHash].isExecuted == false,
            AlreadyExecuted()
        );
    ...
    function voteProposal(
        ...
        LibVoteStorage.ProposalInfo storage vs = LibVoteStorage.voteStorage();
        LibVoteStorage.Proposal storage voteProposalInfo = vs.proposal[
            proposalHash
        ];

        require(
            voteProposalInfo.startTime <= block.timestamp,
            ProposalNotStarted()
        );
        require(voteProposalInfo.to != address(0), InvalidTo());
        require(
            voteProposalInfo.startTime + uint128(voteProposalInfo.duration) >=
                block.timestamp,
            DurationPassed()
        );
        ...
        address signer = signature.length == 0
            ? msg.sender
            : LibValidator.validateSignature(msgHash, signature);
    ...
    function executeProposal(bytes32 proposalHash) external override {
        LibVoteStorage.ProposalInfo storage vs = LibVoteStorage.voteStorage();
        LibVoteStorage.Proposal storage executeProposalInfo = vs.proposal[
            proposalHash
        ];

        require(
            executeProposalInfo.startTime +
                uint128(executeProposalInfo.duration) <
                block.timestamp,
            DurationNotPassed()
        );
        require(
            executeProposalInfo.startTime +
                ...
                SECURITY_WINDOW >
                block.timestamp,
            SecurityWindowPassed()
        );
        require(executeProposalInfo.isExecuted == false, AlreadyExecuted());
        require(
            executeProposalInfo.forCount > executeProposalInfo.againstCount,
            VoteForIsInsufficient()
        );
        ...
        if (executeProposalInfo.isNFT == true) {
            ...
            erc721Contract.safeTransferFrom(
                address(this),
                executeProposalInfo.to,
                executeProposalInfo.data,
                "0x"
            );
        } else {
            executeProposalInfo.isExecuted = true;
